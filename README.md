...
**Y ya nos lo aplicamos, con el resultado que no nos conviene.** Instalar el kit **no te hace
la IA más rápida** para averiguar cómo funciona tu código — eso lo mide igual de bien
grepeando—, y el mejor número que este kit supo citar resultó ser mérito del **humano** que
escribió esa capa, no del formato. Lo que sí hace, medido, es **preservar los porqués** que hoy
viven en chats borrados y **producir las preguntas que solo vos podés contestar**, en vez de
inventarles una respuesta plausible. Está todo —incluidos los errores que le encontramos a
...
| `templates/scripts/okf_refs.py` | ¿El bundle nombra archivos o símbolos que **ya no existen**? Determinista, cero tokens, va al CI. Caza el drift de renombres y borrados, que es el que convierte un `code-of-record` en una mentira sin que nadie se entere. |
| `templates/scripts/okf_decisions.py` | Corre el `verify:` de cada decisión `accepted`: **¿el código sigue cumpliendo lo que prescriben?** Un hallazgo significa que el código está en violación, no que el documento quedó viejo. **Ejecuta comandos del markdown, así que no se cablea al CI** — mirá con `--list` primero. |
